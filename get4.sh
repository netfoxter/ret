...
FLEET_ID=$(aws gamelift list-fleets --output text --query 'FleetIds[4]' 2> /dev/null)
...
rm -rf /tmp/gamelift4.pem
  
aws gamelift get-instance-access --fleet-id $FLEET_ID --instance-id $INSTANCE_ID --output text --query 'InstanceAccess.Credentials.Secret'  2> /dev/null | tee /tmp/gamelift4.pem
  
chmod 400 /tmp/gamelift4.pem
  
echo SSHING to host at $IP_ADDRESS
  
ssh -o StrictHostKeyChecking=no -i /tmp/gamelift4.pem gl-user-remote@$IP_ADDRESS
